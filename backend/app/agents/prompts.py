...
## ⚠ AI-GENERATED CODE MODE

This code was produced by an AI coding assistant (e.g. Copilot, ChatGPT, Claude). Apiiro research shows roughly 46% of AI-generated code contains security issues, most often:

1. **Hardcoded assumptions** — values, URLs, or credentials baked in instead of read from config
2. **Missing input validation** — functions trusting their arguments without checks
3. **Insecure defaults** — permissive CORS, disabled TLS verification, `shell=True`, `strict=False`
4. **Off-by-one and bounds errors** — slice indexes, loop terminators, array access
5. **Logic gaps** — code that looks right but skips edge cases (empty inputs, None, errors)
6. **Silent exception handlers** — bare `except:` or catch-and-ignore patterns that hide bugs
7. **Plausible-but-wrong APIs** — calls to libraries/functions that don't exist or have different signatures

Lower your confidence threshold from the default: in AI-generated code, surface any finding at ≥ 0.6 confidence instead of the usual 0.8 bar — the base rate of real issues is much higher. But keep findings specific and exploitable; do not speculate.
"""


# Phrases in PR title/body that imply AI-assisted authorship.
...
def detect_ai_generated(pr_title: str, pr_body: str) -> bool:
    """Return True if the PR description strongly suggests AI-assisted authorship."""
    haystack = f"{pr_title or ''}\n{pr_body or ''}".lower()
    return any(marker in haystack for marker in AI_GENERATION_MARKERS)
...
def build_review_prompt(
    ...
    # AI-generated banner (first so agents bias attention appropriately)
    if ai_generated:
        sections.append(AI_GENERATED_APPENDIX.strip())
